      ...
      <div className='bg-[#E3D0D8] h-[80%] w-1/4 aspect-auto rounded-[12px] shadow-mainBox p-2 gap-2 flex flex-col overflow-hidden'>
        ...
        <form onSubmit={addTodoHandler} className='flex gap-1'>
          ...
          <button className=' h-9 w-10 bg-[#D0B3C0] border border-[#AEA3B0] rounded-md flex items-center justify-center cursor-pointer'>
            <img src={addIcon} alt="" className='h-[0.9rem]' />
          </button>
        </form>
        <div className='h-full overflow-y-auto'>
          <div className='flex flex-col gap-2 pr-1'>
            ...
            {todos.map((todo, id) => (
              <div key={id} className='bg-[#D0B3C0] rounded-md px-2 py-2 overflow-hidden break-words text-[0.9rem] flex items-center justify-between'>
